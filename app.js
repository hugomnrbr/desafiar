(()=>{
  ...
  async function createCoinPayment(packageId){
    ...
    if(!storeCategoryEnabled('Moedas'))return alert('A compra de QuizCoins está temporariamente desativada.');
  ...
  function bind(){
    ...
    if(S.route==='admin'){bindAdmin();if(!S.adminQuestions.length)setTimeout(loadAdminQuestions,0);if(!S.adminCoinPackages.length)setTimeout(loadAdminCoinPackages,0);if(!S.adminCoinOrders.length)setTimeout(loadAdminCoinOrders,0);if(!S.adminAccounts.length)setTimeout(loadAdminAccounts,0)}
  }
