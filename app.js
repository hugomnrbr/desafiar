(()=>{
  ...
  function top(){
    ...
    const titles={home:'Início',store:'Loja QuizUp',categories:'Categorias',topic:S.topicCategory||'Tópico',ranking:'Ranking',profile:S.profileTargetId===S.user?.id?'Meu Perfil':'Perfil',friends:'Amigos',chat:S.chatTarget?.username||'Chat',notifications:'Notificações',achievements:'Conquistas',contribute:'Criar conteúdo',admin:'Painel'};
    const title=titles[S.route]||'QuizUp';
    return `<header class="classic-top"><button class="top-gear" id="classicHome" title="Voltar ao início">⌂</button><b>${esc(title)}</b><div class="top-right"><button class="top-icon" id="topSearch" title="Pesquisar">⌕</button>${S.user?`<button class="top-icon store-top-btn" id="storeBtn" title="Loja QuizUp">🛒</button><button class="top-icon notify-btn" id="notificationsBtn" title="Notificações">●${S.notificationUnread>0?`<span class="notify-badge">${S.notificationUnread>99?'99+':S.notificationUnread}</span>`:''}</button><button class="top-avatar" id="topProfile">${av(S.profile?.display_name||'J','avatar mini',S.profile?.avatar_url||'')}</button>${S.isAdmin?'<button class="top-icon" id="adminBtn">⚙</button>':''}`:''}</div></header>`;
  }
  ...
  function home(){
    ...
    return `<section class="classic-home">
      ...
      <div class="classic-shortcuts"><button data-go="store">🛒 Loja QuizUp</button><button data-go="friends">♧ Amigos</button><button data-go="achievements">★ Conquistas</button><button data-go="profile">♙ Perfil</button></div>
    </section>`;
  }
  ...
  function money(v){return `R$ ${(Number(v)/100).toFixed(2).replace('.',',')}`}
  async function loadPremiumItems(){
    if(!sb)return;
    const {data,error}=await sb.from('premium_items').select('id,name,category,description,price_cents,icon,asset_url,kind,active').eq('active',true).order('created_at',{ascending:false});
    if(!error&&Array.isArray(data)){S.premiumItems=data.map(x=>({id:x.id,cat:x.category,name:x.name,desc:x.description||'',price:Number(x.price_cents||0),icon:x.icon||'✨',kind:x.kind||({Molduras:'frame',Efeitos:'effect',Títulos:'title',Emojis:'emoji',Avatares:'avatar',Temas:'theme',Badges:'badge'}[x.category]||'theme'),asset_url:x.asset_url||''}));}
  }
  ...
  function premiumStore(){premiumLoad();if(sb&&!S.premiumItems.length)setTimeout(async()=>{await loadPremiumItems();render()},0);const cats=['Molduras','Efeitos','Títulos','Emojis','Avatares','Temas','Badges'];const catalog=premiumCatalog();return `<section class="premium-store"><div class="premium-store-hero"><div><span>⚡ LOJA QUIZUP</span><h1>Crie sua identidade.</h1><p>Personalize seu perfil e deixe sua marca em cada duelo. Nada aqui muda a jogabilidade.</p></div><div class="coin-balance"><b>⚡ ${Number(S.premium.coins||0).toLocaleString('pt-BR')}</b><small>QuizCoins</small><button class="premium-buy-coins" id="buyCoins">+ COMPRAR MOEDAS</button></div></div><div class="vip-card"><div><b>👑 QUIZUP VIP</b><h2>Seu perfil, seu estilo.</h2><p>Nome dourado, moldura VIP, reações exclusivas e efeitos especiais.</p></div><button class="premium-cta" data-premium-buy="vip">ATIVAR VIP • R$ 9,90/mês</button></div><div class="premium-section-head"><b>MAIS VENDIDOS</b><span>Cosméticos • sem vantagem competitiva</span></div><div class="premium-grid">${catalog.slice(0,6).map(premiumCard).join('')}</div><div class="premium-section-head"><b>TODOS OS ITENS</b><span>Escolha uma categoria</span></div><div class="premium-tabs">${cats.map(c=>`<button type="button" data-premium-cat="${esc(c)}">${esc(c)}</button>`).join('')}</div><div class="premium-grid" id="premiumAllGrid">${catalog.map(premiumCard).join('')}</div><div class="premium-pass"><div><span>🏆 TEMPORADA 1</span><h2>PASSSE QUIZUP</h2><p>Desbloqueie molduras, emojis, badges e efeitos enquanto joga.</p></div><button class="premium-cta" data-premium-buy="pass">PASSE VIP • R$ 19,90</button></div><button class="secondary" id="storeHome">← VOLTAR AO INÍCIO</button></section>`}
  function premiumCard(item){const owned=(S.premium.owned||[]).includes(item.id);return `<article class="premium-card ${owned?'owned':''}"><div class="premium-art ${item.kind}">${premiumDisplayArt(item)}</div><div><span class="premium-tag">${esc(item.cat)}</span><h3>${esc(item.name)}</h3><p>${esc(item.desc)}</p></div><button class="premium-item-btn" data-premium-buy="${item.id}">${owned?'✓ ATIVO':money(item.price)}</button></article>`}
  async function persistPremiumProfile(){
    if(!sb||!S.user)return;
    const a=premiumActive();
    const vip=(S.premium.owned||[]).includes('vip');
    const payload={premium_vip:vip,premium_title:a.title||null,premium_frame:a.frame||null,premium_effect:a.effect||null,premium_theme:a.theme||null,premium_avatar:a.avatar||null,premium_badge:a.badge||null};
  ...
  async function premiumBuy(id){
    premiumLoad();
    if(id==='vip'){
      S.premium.owned=[...new Set([...(S.premium.owned||[]),'vip','frame-vip','emoji-troll','effect-electric'])];
      S.premium.active={...S.premium.active,frame:'frame-vip',effect:'effect-electric'};
      premiumSave(); await persistPremiumProfile(); render(); return;
    }
    if(id==='pass'){
      S.premium.owned=[...new Set([...(S.premium.owned||[]),'pass','frame-galaxy','title-master','emoji-elite'])];
      S.premium.active={...S.premium.active,frame:'frame-galaxy',title:'title-master'};
      premiumSave(); await persistPremiumProfile(); render(); return;
    }
    const item=premiumCatalog().find(x=>x.id===id); if(!item||item.price<=0)return;
    if((S.premium.owned||[]).includes(id)){await activateCosmetic(item);return;}
    if(Number(S.premium.coins||0)<item.price)return alert('QuizCoins insuficientes. Na versão de demonstração, use + COMPRAR MOEDAS para testar a loja.');
    S.premium.coins-=item.price;
    S.premium.owned=[...new Set([...(S.premium.owned||[]),id])];
    S.premium.active={...S.premium.active,[item.kind]:id};
    premiumSave(); await persistPremiumProfile(); render();
  }
  function match(){
    if(S.match&&S.matchCountdown>0){const ids=(S.match.player_ids||[]).filter(id=>id!==S.user.id);const opponent=ids[0]?S.matchPlayers[ids[0]]:S.bot;const name=opponent?.username||opponent?.display_name||'Oponente';return `<div class="classic-match-found"><div class="loading-bolt">ϟ</div><div class="classic-match-label">ADVERSÁRIO ENCONTRADO</div><div class="match-vs"><div class="match-player">${av(S.profile?.display_name||'J',`avatar huge premium-frame ${esc(S.profile?.premium_frame||premiumActive().frame||'')}`,S.profile?.avatar_url||'')}<b class="${S.profile?.premium_vip||S.profile?.premium_title?'premium-name-glow':''}">${esc(S.profile?.username||'Você')} ${S.profile?.premium_vip?'<em class="premium-badge">VIP</em>':''}</b>${premiumItemById(S.profile?.premium_effect||premiumActive().effect)?.icon?`<span class="countdown-cosmetic-effect">${esc(premiumItemById(S.profile?.premium_effect||premiumActive().effect).icon)} efeito</span>`:''}</div><div class="vs-glow">VS</div><div class="match-player">${av(opponent?.username||'R',`avatar huge premium-frame ${esc(opponent?.premium_frame||'')}`,opponent?.avatar_url||'')}<b class="${opponent?.premium_vip||opponent?.premium_title?'premium-name-glow':''}">${esc(name)} ${opponent?.premium_vip?'<em class="premium-badge">VIP</em>':''}</b>${premiumItemById(opponent?.premium_effect)?.icon?`<span class="countdown-cosmetic-effect">${esc(premiumItemById(opponent?.premium_effect).icon)} efeito</span>`:''}</div></div><div class="countdown-label">Começando em</div><div class="countdown-number">${S.matchCountdown}</div></div>`}
  ...
  function subscribeMatch(){
    ...
    S.realtime=sb.channel(`match-${S.matchId}`).on('postgres_changes',{event:'UPDATE',schema:'public',table:'matches',filter:`id=eq.${S.matchId}`},async payload=>{await syncMatch(payload.new)}).on('postgres_changes',{event:'INSERT',schema:'public',table:'match_reactions',filter:`match_id=eq.${S.matchId}`},payload=>{const r=payload.new;if(!S.matchReactions.some(x=>x.id===r.id)){S.matchReactions.push(r);if(S.matchReactions.length>6)S.matchReactions.shift();S.lastReaction=r.emoji||'';render();setTimeout(()=>{S.lastReaction='';render()},1800)}}).subscribe();
  ...
  function asyncGame(){
    ...
    return `<div class="neon-game async-game"><div class="neon-duel-head"><div class="neon-player"><div class="neon-avatar-wrap">${av(S.profile?.display_name||'V',`avatar neon-avatar premium-frame ${esc(S.profile?.premium_frame||premiumActive().frame||'')}`,S.profile?.avatar_url||'')}</div><div class="neon-player-copy"><span>VOCÊ ${premiumBadge()}</span><b>${esc(S.profile?.username||'Você')}</b><small class="premium-game-title">${esc(premiumActive().title||'')}</small><strong>${mine}<small> pts</small></strong></div></div><div class="neon-vs">VS</div><div class="neon-player rival"><div class="neon-player-copy"><span>AMIGO</span><b>${esc(opp.username||'Amigo')}</b><strong>${rival}<small> pts</small></strong></div><div class="neon-avatar-wrap">${av(opp.username||'R',`avatar neon-avatar premium-frame ${esc(opp.premium_frame||'')}`,opp.avatar_url||'')}</div></div></div><div class="neon-progress"><div><i style="width:${myPct}%"></i></div><em></em><div><i style="width:${oppPct}%"></i></div></div><div class="neon-rounds">${Array.from({length:7},(_,i)=>`<span class="${i<myProgress?'done':i===myProgress?'current':''}">${i<myProgress?'✓':i+1}</span>`).join('')}</div><div class="neon-question-card"><div class="neon-question-meta"><span>PERGUNTA ${myProgress+1} DE 7</span><div class="neon-timer" id="tm">${S.seconds}</div></div>${q.image_url?`<img class="neon-question-image" src="${esc(q.image_url)}" alt="">`:''}<p>${esc(q.question_text||'')}</p></div><div class="neon-answers">${(q.options||[]).map((a,i)=>{const selected=myAnswer&&Number(myAnswer.answer)===i;const correct=Number(q.correct_index)===i;const cls=myAnswer&&(correct||selected)?(correct?'answer-correct':selected?'answer-wrong':''):'';return `<button type="button" class="neon-answer ${cls}" data-async-a="${i}" ${S.asyncAnswered?'disabled':''}><span>${'ABCD'[i]}</span><label>${esc(a)}</label>${myAnswer&&correct?'<b>✓</b>':(myAnswer&&selected&&!myAnswer.correct?'<b>✕</b>':'')}</button>`}).join('')}</div>${feedback}${S.asyncAnswered?'<div class="neon-wait">✓ RESPOSTA ENVIADA • AGUARDANDO</div>':''}<div class="async-note">Desafio assíncrono • cada jogador responde no seu ritmo</div></div>`;
  }
  ...
  async function finishAsyncOnline(){if(S.route==='result')return;const mine=getScore(S.user.id);const opp=(S.match?.player_ids||[]).find(x=>x!==S.user.id);const oppScore=getScore(opp);const outcome=mine===oppScore?'draw':mine>oppScore?'win':'loss';const win=outcome==='win';if(sb){const {data:existing}=await sb.from('game_results').select('id').eq('match_id',S.matchId).eq('user_id',S.user.id).maybeSingle();if(!existing){await sb.from('game_results').insert({user_id:S.user.id,category:S.category,mode:'1v1',score:mine,won:win,outcome,match_id:S.matchId});if(outcome!=='draw'){const vip=!!S.profile?.premium_vip||!!S.premium?.owned?.includes('vip');const xpScore=vip?Math.round(mine*1.25):mine;await sb.rpc('apply_game_result',{p_user_id:S.user.id,p_category:S.category,p_score:xpScore,p_won:win});};await loadProfile()}}S.myScore=mine;S.rankRows=null;S.profileDetails=null;S.route='result';render()}
  ...
  function game(){
    ...
    return `<div class="neon-game"><div class="neon-duel-head"><div class="neon-player"><div class="neon-avatar-wrap">${av(S.profile?.display_name||'V',`avatar neon-avatar premium-frame ${esc(S.profile?.premium_frame||premiumActive().frame||'')}`,S.profile?.avatar_url||'')}${myReaction?`<span class="avatar-reaction-bubble mine">${esc(myReaction.emoji)}</span>`:''}</div><div class="neon-player-copy"><span>VOCÊ ${premiumBadge()}</span><b>${esc(S.profile?.username||'Você')}</b><small class="premium-game-title">${esc(premiumActive().title||'')}</small><strong>${mine}<small> pts</small></strong></div></div><div class="neon-vs">VS</div><div class="neon-player rival"><div class="neon-player-copy"><span>OPONENTE</span><b>${esc(opp.username||'Oponente')}</b><strong>${rival}<small> pts</small></strong></div><div class="neon-avatar-wrap">${av(opp.username||'R',`avatar neon-avatar premium-frame ${esc(opp.premium_frame||'')}`,opp.avatar_url||'')}${oppReaction?`<span class="avatar-reaction-bubble opponent">${esc(oppReaction.emoji)}</span>`:''}</div></div></div><div class="neon-progress"><div><i style="width:${pctMe}%"></i></div><em></em><div><i style="width:${pctOpp}%"></i></div></div><div class="neon-rounds">${Array.from({length:7},(_,i)=>`<span class="${i< S.questionIndex?'done':i===S.questionIndex?'current':''}">${i<S.questionIndex?'✓':i+1}</span>`).join('')}</div><div class="neon-question-card"><div class="neon-question-meta"><span>PERGUNTA ${progress} DE 7</span><div class="neon-timer" id="tm">${S.seconds}</div></div>${q.image_url?`<img class="neon-question-image" src="${esc(q.image_url)}" alt="">`:''}<p>${esc(q.question_text||'')}</p></div><div class="neon-answers">${(q.options||[]).map((a,i)=>{const selected=myAnswer&&Number(myAnswer.answer)===i;const isCorrect=Number(q.correct_index)===i;const cls=answeredNow&&isCorrect?'answer-correct':(selected&&!myAnswer?.correct?'answer-wrong':(selected?'selected':''));return `<button type="button" class="neon-answer ${cls}" data-a="${i}" ${answeredNow?'disabled':''}><span>${'ABCD'[i]}</span><label>${esc(a)}</label>${answeredNow&&isCorrect?'<b>✓</b>':(selected&&!myAnswer?.correct?'<b>✕</b>':'')}</button>`}).join('')}</div>${feedback}${S.waiting?'<div class="neon-wait">✓ RESPOSTA ENVIADA • AGUARDANDO O OPONENTE</div>':''}<div class="match-reaction-bar"><button type="button" id="matchEmojiBtn" class="match-emoji-open">😊 Reagir</button></div>${reactionPicker}</div>`;
  }
  ...
  async function answer(i){
    ...
    if(fresh){S.match=fresh;S.myScore=Number(fresh.scores?.[S.user.id]||0);}
    render()
  }
  ...
  async function finishOnline(){if(S.route==='result')return;const mine=getScore(S.user.id);const teamA=totalTeam(S.match.team_a),teamB=totalTeam(S.match.team_b);const rivalScore=S.mode==='2v2'?(S.match.team_a.includes(S.user.id)?teamB:teamA):Math.max(...S.match.player_ids.filter(x=>x!==S.user.id).map(getScore),0);const outcome=mine===rivalScore?'draw':mine>rivalScore?'win':'loss';const win=outcome==='win';if(sb){const {data:existing}=await sb.from('game_results').select('id').eq('match_id',S.matchId).eq('user_id',S.user.id).maybeSingle();if(!existing){await sb.from('game_results').insert({user_id:S.user.id,category:S.category,mode:S.mode,score:mine,won:win,outcome,match_id:S.matchId});if(outcome!=='draw'){const vip=!!S.profile?.premium_vip||!!S.premium?.owned?.includes('vip');const xpScore=vip?Math.round(mine*1.25):mine;await sb.rpc('apply_game_result',{p_user_id:S.user.id,p_category:S.category,p_score:xpScore,p_won:win});};await loadProfile();}}S.myScore=mine;S.rankRows=null;S.profileDetails=null;S.route='result';render()}
  ...
  function result(){
    const mine=getScore(S.user.id)||S.myScore;const opponents=(S.match?.player_ids||[]).filter(x=>x!==S.user.id);const opp=opponents[0]?S.matchPlayers[opponents[0]]:(S.bot||{});const oppScore=S.mode==='2v2'?((S.match?.team_a||[]).includes(S.user.id)?totalTeam(S.match.team_b):totalTeam(S.match.team_a)):(opponents[0]?getScore(opponents[0]):getScore(S.bot?.id));if(!S.resultSoundPlayed){sound(outcome==='win'?'win':outcome==='draw'?'match':'lose');S.resultSoundPlayed=true}if(sb&&S.matchId&&!S.botMode&&!S.resultPresenceTimer)setTimeout(startResultPresence,0);const outcome=mine===oppScore?'draw':mine>oppScore?'win':'loss',win=outcome==='win',perfect=mine===160;
    const chart=detailChart();
    return `<div class="classic-result"><div class="result-kicker">RESULTADOS</div><h1>${outcome==='win'?'VITÓRIA':outcome==='draw'?'EMPATE':'DERROTA'}</h1><div class="result-players"><div>${av(S.profile?.display_name||'J','avatar big',S.profile?.avatar_url||'')}<b>${esc(S.profile?.username||'Você')}</b><strong class="${win?'green':'red'}">${mine}</strong></div><span>VS</span><div>${av(opp?.username||'R','avatar big',opp?.avatar_url||'')}<b>${esc(opp?.username||'Oponente')}</b><strong class="${outcome==='draw'?'yellow':'red'}">${oppScore}</strong></div></div><div class="result-metrics"><div><b>${mine}</b><small>PONTUAÇÃO</small></div><div><b>${S.match?.answers?Object.values(S.match.answers).filter(x=>x&&x.score).length:0}</b><small>RESPOSTAS</small></div><div><b>${perfect?'160':'+'+mine}</b><small>XP DA PARTIDA</small></div></div>${perfect?'<div class="perfect-score">🏆 PONTUAÇÃO PERFEITA!</div>':''}<div class="classic-xp"><div class="xp-circle"><span>XP</span><b>${S.botMode?'+0':'+'+mine}</b></div><div><b>${S.botMode?'Partida contra robô':'Experiência conquistada'}</b><small>${outcome==='win'?(S.profile?.premium_vip?'Vitória + bônus VIP de 25% no XP.':'Você venceu este confronto.') :outcome==='draw'?'Os dois terminaram com a mesma pontuação.':'Continue jogando para subir no ranking.'}</small></div></div><div class="result-detail"><b>DETALHES</b>${chart}</div><div class="result-actions">${!S.botMode&&opponents.length?`<button class="result-btn play" id="rematch">Jogar</button><button class="result-btn chat" id="resultChat">Chat</button><button class="result-btn publish" id="publishResult">Publicar</button>`:''}<button class="secondary" id="again">JOGAR OUTRA</button><button class="secondary" id="home">VOLTAR AO INÍCIO</button>${S.rematchMessage?`<div class="notice center rematch-notice">${esc(S.rematchMessage)}</div><button class="secondary" id="findAnother">PROCURAR OUTRO</button>`:''}</div></div>`;
  }
  ...
  async function loadProfileDetails(id){
    ...
    let {data:p,error}=await sb.from('profiles').select('id,username,display_name,avatar_url,xp,level,wins,losses,streak,premium_vip,premium_frame,premium_effect,premium_theme,premium_avatar,premium_title,premium_badge').eq('id',id).single();
  ...
  async function createPremiumItem(){
    ...
    const name=$('#premiumItemName')?.value.trim();const category=$('#premiumItemCategory')?.value;const price=Number($('#premiumItemPrice')?.value||0);const icon=$('#premiumItemIcon')?.value.trim()||'✨';const description=$('#premiumItemDesc')?.value.trim()||null;const file=$('#premiumItemFile')?.files?.[0]||null;
    ...
    try{
      ...
      const id=`custom-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;const {error}=await sb.from('premium_items').insert({id,name,category,description,price_cents:price,icon,asset_url,kind,active:true,created_by:S.user.id});if(error)throw error;
  ...
  async function loadProfile(){
    if(!sb||!S.user)return;
    let {data,error}=await sb.from('profiles').select('id,username,display_name,avatar_url,xp,level,wins,losses,streak,role,premium_vip,premium_frame,premium_effect,premium_theme,premium_avatar,premium_title,premium_badge').eq('id',S.user.id).single();
    ...
    S.profile=data||{username:S.user.email?.split('@')[0],display_name:S.user.email?.split('@')[0]};premiumLoad();
  ...
  async function sendMatchReaction(emoji){if(!sb||!S.matchId||!S.user||!emoji)return;S.emojiOpen=false;S.lastReaction=emoji;const {error}=await sb.from('match_reactions').insert({match_id:S.matchId,sender_id:S.user.id,emoji});if(error)console.warn('reaction',error.message);render()}
  function bind(){
  ...
  premiumLoad();async function init(){if(!sb){S.user=null;S.profile=null;S.profileTargetId=null;return go('login')}const {data:{session}}=await sb.auth.getSession();S.accessToken=session?.access_token||null;const {data:{user}}=await sb.auth.getUser();S.user=user||null;if(!S.user)return go('login');subscribeChallenges();subscribeNotifications();await loadProfile();await loadCategories();await loadPremiumItems();await loadNotifications();S.achievementsRows=null;S.profileTargetId=S.user.id;go('home')}
