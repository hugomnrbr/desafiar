(()=>{
  ...
  function view(){if(!S.user&&!['login','signup'].includes(S.route))return login();if(S.route==='login')return login();if(S.route==='signup')return signup();if(S.route==='home')return home();if(S.route==='store')return premiumStore();if(S.route==='categories')return categories();if(S.route==='topic')return topic();if(S.route==='contribute')return contribute();if(S.route==='match')return match();if(S.route==='game')return game();if(S.route==='async-game')return asyncGame();if(S.route==='result')return result();if(S.route==='ranking')return rank();if(S.route==='profile')return profile();if(S.route==='friends')return friends();if(S.route==='chat')return chat();if(S.route==='support')return support();if(S.route==='notifications')return notificationsView();if(S.route==='admin')return admin();return achievements()}
  ...
  function home(){
    ...
    return `<section class="classic-home">
      ...
      <div class="classic-shortcuts"><button data-go="store">🛒 Loja QuizUp</button><button data-go="friends">♧ Amigos</button><button data-go="achievements">★ Conquistas</button><button data-go="support">💬 Suporte</button><button data-go="profile">♙ Perfil</button></div>
    </section>`;
  }
  ...
  function bind(){
    ...
    $('#logoutTop')?.addEventListener('click',async()=>{if(confirm('Deseja sair da sua conta?'))$('#logout')?.click()});$('#logout')?.addEventListener('click',async()=>{if(S.challengeRealtime&&sb){sb.removeChannel(S.challengeRealtime);S.challengeRealtime=null}if(S.notificationRealtime&&sb){sb.removeChannel(S.notificationRealtime);S.notificationRealtime=null}if(S.sessionTimer)clearInterval(S.sessionTimer);if(sb&&S.sessionId){await sb.rpc('release_quizup_session',{p_session_id:S.sessionId}).catch(()=>{});}if(sb)await sb.auth.signOut();S.sessionId=null;S.user=null;S.profile=null;go('login')});
  ...
  async function claimSingleSession(){if(!sb||!S.user)return false;S.sessionId=localStorage.getItem('quizup_session_id')||crypto.randomUUID();localStorage.setItem('quizup_session_id',S.sessionId);const {data,error}=await sb.rpc('claim_quizup_session',{p_session_id:S.sessionId});if(error||data!==true){alert('Esta conta já foi conectada neste momento. A sessão anterior foi encerrada.');await sb.auth.signOut();return false}if(S.sessionTimer)clearInterval(S.sessionTimer);S.sessionTimer=setInterval(async()=>{if(!sb||!S.user||!S.sessionId)return;const {data:ok}=await sb.rpc('touch_quizup_session',{p_session_id:S.sessionId});if(ok===false){clearInterval(S.sessionTimer);S.sessionTimer=null;alert('Sua conta foi conectada em outro aparelho. Esta sessão foi encerrada.');await sb.auth.signOut();S.user=null;S.profile=null;go('login')}},8000);return true}
premiumLoad();async function init(){if(!sb){S.user=null;S.profile=null;S.profileTargetId=null;return go('login')}const {data:{session}}=await sb.auth.getSession();S.accessToken=session?.access_token||null;const {data:{user}}=await sb.auth.getUser();S.user=user||null;if(!S.user)return go('login');if(!(await claimSingleSession()))return;subscribeChallenges();subscribeNotifications();await loadProfile();await loadCategories();await loadPremiumItems();await loadStoreSettings();await loadCoinPackages();await loadMyCoinOrders();await checkPaymentReturn();await loadNotifications();S.achievementsRows=null;S.profileTargetId=S.user.id;if(S.paymentReturn){go('store');return}go('home')}
