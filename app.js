(()=>{
  ...
  function premiumStore(){
    premiumLoad();
    if(sb&&!S.premiumItems.length)setTimeout(async()=>{await loadPremiumItems();await loadStoreSettings();await loadCoinPackages();await loadMyCoinOrders();render()},0);
    const cats=['Molduras','Efeitos','Títulos','Emojis','Avatares','Temas','Badges'];const catalog=premiumCatalog();const off=!S.storeSettings?.enabled;
    const paymentNotice=S.paymentReturn?`<div class="notice payment-return ${S.paymentReturn.status==='approved'?'payment-ok':''}">${S.paymentReturn.status==='approved'?'✅ Pagamento aprovado. O webhook confirmará e liberará as Coins automaticamente.':S.paymentReturn.status==='pending'?'⏳ Pagamento pendente. Assim que o Mercado Pago confirmar, suas Coins serão creditadas.':'ℹ️ Retornamos do checkout. Confira o status da compra abaixo.'}</div>`:'';
    const orders=(S.myCoinOrders||[]).slice(0,5).map(o=>`<div class="coin-order-row"><span>⚡ ${Number(o.coins).toLocaleString('pt-BR')}</span><small>R$ ${(Number(o.amount_cents||0)/100).toFixed(2).replace('.',',')} • ${o.status==='approved'?'APROVADO':o.status==='pending'?'PENDENTE':o.status==='rejected'?'RECUSADO':esc(o.status)}</small></div>`).join('');
    return `<section class="premium-store"><div class="premium-store-hero"><div><span>⚡ LOJA</span><h1>Crie sua identidade.</h1><p>Personalize seu perfil e deixe sua marca em cada duelo. Nada aqui muda a jogabilidade.</p>${off?'<div class="store-disabled">🛑 Compras temporariamente desativadas pelo administrador.</div>':''}</div><div class="coin-balance"><b>⚡ ${Number(S.profile?.coins??S.premium.coins??0).toLocaleString('pt-BR')}</b><small>QuizCoins</small><button class="premium-buy-coins" id="buyCoins" ${storeCategoryEnabled('Moedas')?'':'disabled'}>+ COMPRAR MOEDAS</button></div></div>${paymentNotice}<div class="coin-shop-card"><div class="premium-section-head"><b>⚡ COMPRAR QUIZCOINS</b><span>Pagamento seguro pelo Mercado Pago</span></div><div class="coin-pack-grid">${(S.coinPackages||[]).map(x=>`<article class="coin-pack"><strong>⚡ ${Number(x.coins).toLocaleString('pt-BR')}</strong><span>${esc(x.name||'Pacote de Coins')}</span><b>R$ ${(Number(x.price_cents||0)/100).toFixed(2).replace('.',',')}</b><button class="premium-item-btn" type="button" data-coin-package="${x.id}" ${storeCategoryEnabled('Moedas')?'':'disabled'}>${storeCategoryEnabled('Moedas')?'COMPRAR':'INDISPONÍVEL'}</button></article>`).join('')||'<div class="muted">O administrador ainda não cadastrou pacotes.</div>'}</div></div>${orders?`<div class="card pad coin-orders"><div class="premium-section-head"><b>🧾 MINHAS COMPRAS</b><span>Últimas transações</span></div>${orders}</div>`:''}${S.coinLedgerRows?.length?`<div class="card pad coin-ledger"><div class="premium-section-head"><b>⚡ HISTÓRICO DE QUIZCOINS</b><span>Entradas e gastos</span></div>${S.coinLedgerRows.slice(0,12).map(x=>`<div class="coin-ledger-row"><span class="${Number(x.amount)>=0?'positive':'negative'}">${Number(x.amount)>=0?'+':''}${Number(x.amount).toLocaleString('pt-BR')} ⚡</span><span>${esc(x.description||x.source_type||'Movimentação')}</span><small>${new Date(x.created_at).toLocaleString('pt-BR')}</small></div>`).join('')}</div>`:''}<div class="vip-card"><div><b>👑 QUIZUP VIP</b><h2>Seu perfil, seu estilo.</h2><p>Nome dourado, moldura VIP, reações exclusivas e bônus de XP.</p></div><button class="premium-cta" data-premium-buy="vip" ${storeCategoryEnabled('VIP')?'':'disabled'}>ATIVAR VIP • 9.900 ⚡</button></div><div class="premium-section-head"><b>MAIS VENDIDOS</b><span>Cosméticos • sem vantagem competitiva</span></div><div class="premium-grid">${catalog.slice(0,6).map(premiumCard).join('')}</div><div class="premium-section-head"><b>TODOS OS ITENS</b><span>Escolha uma categoria</span></div><div class="premium-tabs">${cats.map(c=>`<button type="button" data-premium-cat="${esc(c)}">${esc(c)}</button>`).join('')}</div><div class="premium-grid" id="premiumAllGrid">${catalog.map(premiumCard).join('')}</div><div class="premium-pass"><div><span>🏆 TEMPORADA 1</span><h2>PASSE QUIZUP</h2><p>Desbloqueie molduras, emojis, badges e efeitos enquanto joga.</p></div><button class="premium-cta" data-premium-buy="pass" ${storeCategoryEnabled('Passe')?'':'disabled'}>PASSE VIP • 1.990 ⚡</button></div><button class="secondary" id="storeHome">← VOLTAR AO INÍCIO</button></section>`}
  ...
  function game(){
    ...
    return `<div class="neon-game"><div class="neon-duel-head"><div class="neon-player"><div class="neon-avatar-wrap">${profileAvatarMarkup(S.profile,'avatar neon-avatar')}<span class="age18-badge">+18</span>${myReaction?`<span class="avatar-reaction-bubble mine">${esc(myReaction.emoji)}</span>`:''}<strong class="score-under-avatar">${mine}<small> pts</small>${myRoundGain>0?`<em>+${myRoundGain}</em>`:''}</strong></div><div class="neon-player-copy"><span>VOCÊ ${premiumBadge()}</span><b>${esc(S.profile?.username||'Você')}</b><small class="premium-game-title">${esc(premiumActive().title||'')}</small></div></div><div class="neon-vs">VS</div><div class="neon-player rival"><div class="neon-player-copy"><span>OPONENTE</span><b>${esc(opp.username||'Oponente')}</b></div><div class="neon-avatar-wrap">${profileAvatarMarkup(opp,'avatar neon-avatar')}<span class="age18-badge">+18</span>${oppReaction?`<span class="avatar-reaction-bubble opponent">${esc(oppReaction.emoji)}</span>`:''}<strong class="score-under-avatar">${rival}<small> pts</small></strong></div></div></div><div class="neon-progress"><div><i style="width:${pctMe}%"></i></div><em></em><div><i style="width:${pctOpp}%"></i></div></div><div class="neon-rounds">${Array.from({length:7},(_,i)=>`<span class="${i< S.questionIndex?'done':i===S.questionIndex?'current':''}">${i<S.questionIndex?'✓':i+1}</span>`).join('')}</div><div class="neon-question-card"><div class="neon-question-meta"><span>PERGUNTA ${progress} DE 7</span><div class="neon-timer" id="tm">${S.seconds}</div></div>${q.image_url?`<img class="neon-question-image" src="${esc(q.image_url)}" alt="">`:''}<p>${esc(q.question_text||'')}</p></div><div class="neon-answers">${(q.options||[]).map((a,i)=>{const selected=myAnswer&&Number(myAnswer.answer)===i;const isCorrect=Number(q.correct_index)===i;const cls=answeredNow&&isCorrect?'answer-correct':(selected&&!myAnswer?.correct?'answer-wrong':(selected?'selected':''));return `<button type="button" class="neon-answer ${cls}" data-a="${i}" ${answeredNow?'disabled':''}><span>${'ABCD'[i]}</span><label>${esc(a)}</label>${answeredNow&&isCorrect?'<b>✓</b>':(selected&&!myAnswer?.correct?'<b>✕</b>':'')}</button>`}).join('')}</div>${feedback}${S.waiting?'<div class="neon-wait">✓ RESPOSTA ENVIADA • AGUARDANDO O OPONENTE</div>':''}<div class="match-reaction-bar"><button type="button" id="matchEmojiBtn" class="match-emoji-open">😊 Reagir</button></div>${reactionPicker}</div>`;
  }
  ...
  async function createPremiumItem(){
    ...
    try{
      ...
      const kind={Molduras:'frame',Efeitos:'effect',Títulos:'title',Emojis:'emoji',Avatares:'avatar',Temas:'theme',Badges:'badge'}[category]||'theme';
  ...
  async function loadProfile(){
    if(!sb||!S.user)return;
    let {data,error}=await sb.from('profiles').select('id,username,display_name,avatar_url,xp,level,wins,losses,streak,role,coins,premium_vip,premium_frame,premium_effect,premium_theme,premium_avatar,premium_title,premium_badge').eq('id',S.user.id).single();
    ...
    S.isAdmin=S.profile.role==='admin';
  }
  ...
  function bind(){
    ...
    $('#avatarFile')?.addEventListener('change',e=>uploadAvatar(e.target.files?.[0]));$('#profileBlock')?.addEventListener('click',()=>toggleBlockUser(S.profileTargetId));$('#profileMute')?.addEventListener('click',()=>toggleMuteUser(S.profileTargetId));
