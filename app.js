(()=>{
  ...
  function contribute(){
    ...
    const n=10;
    const qcards=Array.from({length:n},(_,i)=>`<div class="category-question-card"><div class="cq-head"><b>Pergunta ${i+1} de ${n}</b><span>4 respostas • 1 correta</span></div><select id="cqType${i}" class="cq-type"><option value="text">Somente texto</option><option value="image">Pergunta com imagem</option></select><div id="cqImageWrap${i}" class="cq-image-wrap" hidden><label class="upload-image-label" for="cqImage${i}">📷 Escolher imagem do celular</label><input id="cqImage${i}" type="file" accept="image/jpeg,image/png,image/webp,image/gif" hidden><div id="cqImageName${i}" class="muted">Nenhuma imagem selecionada</div><img id="cqImagePreview${i}" class="cq-image-preview" hidden alt="Prévia da imagem da pergunta"><small>A imagem será enviada automaticamente para o Supabase Storage. Máximo: 5 MB.</small></div><textarea class="cq-text" id="cqText${i}" required maxlength="300" placeholder="Digite a pergunta ${i+1}"></textarea><div class="cq-options"><input id="cq0_${i}" required maxlength="160" placeholder="A) Resposta"><input id="cq1_${i}" required maxlength="160" placeholder="B) Resposta"><input id="cq2_${i}" required maxlength="160" placeholder="C) Resposta"><input id="cq3_${i}" required maxlength="160" placeholder="D) Resposta"></div><select id="cqCorrect${i}" required><option value="0">A é a correta</option><option value="1">B é a correta</option><option value="2">C é a correta</option><option value="3">D é a correta</option></select></div>`).join('');
    return `<section class="contribute-page"><div class="title">Criar novo tópico</div><div class="subtitle">Crie uma categoria original e prepare <b>pelo menos 10 perguntas</b>. Escolha entre perguntas somente de texto ou perguntas com imagem. As imagens são enviadas diretamente para o Supabase Storage.</div><form id="categoryPackageForm" class="form"><div class="card pad"><h3>1. Dados da categoria</h3><input id="packageCatName" required maxlength="60" placeholder="Nome da categoria"><input id="packageCatIcon" maxlength="4" placeholder="Ícone, ex.: ⚽"><textarea id="packageCatDesc" maxlength="180" placeholder="Descrição da categoria"></textarea></div><div class="card pad" style="margin-top:12px"><h3>2. Suas 10 perguntas</h3><p class="muted">As 10 perguntas abaixo são obrigatórias. Para usar uma imagem, escolha “Pergunta com imagem” e selecione uma foto do celular.</p>${qcards}</div><div class="card pad" style="margin-top:12px"><div class="notice"><b>Antes de enviar:</b> confira as 10 perguntas, as 4 alternativas e a resposta correta. O administrador receberá o pacote completo para aprovação.</div><button class="primary" type="submit" id="submitCategoryPackage">ENVIAR CATEGORIA E 10 PERGUNTAS</button></div></form><button class="secondary" id="backCategories">← VOLTAR ÀS CATEGORIAS</button></section>`;
  }
  function match(){
